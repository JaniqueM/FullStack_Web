"""
STEP 5: DSA - Compare Linear Search vs Dictionary Lookup
Run this file to see the speed difference between the two methods.
...
# Goes through every single record one by one until it finds the right ID
# Like looking through every page of a book to find a word

def linear_search(transactions, target_id):
    """Search through the list one by one until we find the ID."""
...
# Stores all records in a dictionary where the key is the ID
...
A: Linear search checks every record one by one until it finds
   the right ID. If the record is near the end, it has to check
   all records before it. This is called O(n) time complexity —
   ...
   This is called O(1) time complexity — it takes the same amount
   of time no matter how many records you have.

Q: Can you suggest another data structure or algorithm?

A: Yes! Here are two better options:
   ...
   2. Hash Map (same as Python dict) — Already what we are using!
      Best for exact ID lookups with O(1) average time.
